...
export const endInteraction = async (
  ...
  if (messages) {
    const { text } = await generateText({
      model: openai(
        (process.env.SUMMARIZER_AGENT_MODEL as string) || "gpt-5.1"
      ),
      system:
...
export const summarizeInteractions = async (
  interactionId: string
): Promise<string> => {
  const interaction: Interaction = await getInteraction(interactionId);
  const messages: Message[] = await listMessagesByInteractionId(interaction.id);
  ...
  return "No messages in Interaction found.";
};
